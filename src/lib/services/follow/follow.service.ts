...
export class FollowService {
      ...
      .subscribe(
        ...
        (error) => {
          ...
          if (errorString.includes(this.RULE_ERROR_FOLLOW_ENTRY_ALREADY_EXISTS)) {
            ...
            this.tracking.log(`profile : ${isFollow ? "follow" : "unfollow"}`, { status: "error", error: parsedError });
  ...
  _handleSuccessfulFollow(followedPubKeyBase58Check: string) {
    this.tracking.log("profile : follow", { status: "success", targetPublicKey: followedPubKeyBase58Check });
  ...
  _handleSuccessfulUnfollow(followedPubKeyBase58Check: string) {
    this.tracking.log("profile : unfollow", { status: "success", targetPublicKey: followedPubKeyBase58Check });
