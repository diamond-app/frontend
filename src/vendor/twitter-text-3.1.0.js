...
(function (global, factory) {
  ...
  function linkToTextWithSymbol(entity, symbol, text, attributes, options) {
    var taggedSymbol = options.symbolTag
      ? "<".concat(options.symbolTag, ">").concat(symbol, "</").concat(options.symbolTag, ">")
      : symbol;
    text = htmlEscape(text);
    var taggedText = options.textWithSymbolTag
      ? "<".concat(options.textWithSymbolTag, ">").concat(text, "</").concat(options.textWithSymbolTag, ">")
      : text;
  ...
  for (var collections = getKeys(DOMIterables), i$1 = 0; i$1 < collections.length; i$1++) {
    ...
    if (proto$1) {
      ...
      if (explicit)
        for (key in es6_array_iterator) if (!proto$1[key]) redefine(proto$1, key, es6_array_iterator[key], true);
    }
  }
  ...
  // Licensed under the Apache License, Version 2.0
  // http://www.apache.org/licenses/LICENSE-2.0
  var validateUrlUnicodeDomainSegment =
  ...
  var validateUrlUnicodeDomainTld =
    /(?:(?:[a-z]|[^\u0000-\u007f])(?:(?:[a-z0-9\-]|[^\u0000-\u007f])*(?:[a-z0-9]|[^\u0000-\u007f]))?)/i;
